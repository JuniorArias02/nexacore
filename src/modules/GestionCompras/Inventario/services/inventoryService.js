...
export const inventoryService = {
    ...
    searchProductos: async (query) => {
        const response = await api.get('/gestion-compras/cp-productos-servicios/buscar', { params: { termino: query } });
        return response.data; // Return full response object to check 'source' and 'objeto'
    },

    searchProductosExterno: async (query) => {
        const response = await api.get('/gestion-compras/cp-productos-servicios/buscar-externo', { params: { termino: query } });
    ...
    getCpDependencias: async (sedeId) => {
        const response = await api.get('/gestion-compras/cp-dependencias', { params: { sede_id: sedeId } });
        return response.data.objeto;
    },
