...
export default function ActasEntregaListPage() {
    ...
    const loadEntregas = async () => {
        try {
            setLoading(true);
            const data = await actasEntregaService.getAll();
            setEntregas(data || []);
        } catch (error) {
            console.error('ActasEntregaListPage: Error loading entregas:', error);
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                await actasEntregaService.delete(id);
                setEntregas(prev => prev.filter(item => item.id !== id));
                Swal.fire({
                    title: '¡Eliminado!',
                    text: 'El acta de entrega ha sido eliminada.',
                    icon: 'success',
                    timer: 2000,
    ...
    const filteredItems = entregas.filter(item =>
        ...
        item.funcionario?.cedula?.toLowerCase().includes(searchTerm.toLowerCase())
    );
            ...
            <div className="relative overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-violet-600 via-indigo-600 to-blue-700 p-8 md:p-12 text-white shadow-2xl mb-10 group">
                <div className="relative z-10">
                    ...
                    <h1 className="text-4xl md:text-5xl font-black tracking-tight text-white mb-4 drop-shadow-sm">
                        Actas de <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-200 to-white">Entrega</span>
                    </h1>
                    <p className="text-indigo-100 max-w-2xl text-lg font-medium leading-relaxed opacity-90">
                        Registro y trazabilidad de hardware y periféricos asignados al personal.
                    </p>

                    <div className="mt-10 flex flex-wrap gap-4">
                        <Link
                            to="/gestion-sistemas/actas-entrega/nuevo"
                    ...
                    <table className="min-w-full divide-y divide-gray-100">
                        <thead className="bg-slate-50/50">
                            <tr>
                                ...
                                <th className="px-6 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Equipo Principal</th>
                            ...
                            {loading ? (
                                <tr>
                                    <td colSpan="5" className="py-24 text-center">
                                        ...
                                        <span className="mt-4 block text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Cargando Actas...</span>
                                    </td>
                                </tr>
                                    ...
                                    <tr key={item.id} className="hover:bg-indigo-50/30 transition-all duration-200 group">
                                        <td className="px-6 py-5">
                                            <div className="flex flex-col">
                                                <span className="text-xs font-black text-slate-400 tracking-widest uppercase">ID: {item.id}</span>
