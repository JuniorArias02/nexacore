...
export default function BuzonAgenteList() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const data = await buzonSugerenciasService.getPendientes();
            ...
            <div className="relative overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-violet-600 via-indigo-600 to-blue-700 p-8 md:p-12 text-white shadow-2xl mb-10 group">
                <div className="relative z-10">
                    ...
                    <p className="text-indigo-100 max-w-2xl text-lg font-medium leading-relaxed opacity-90">
                        Gestiona y atiende los tickets reportados por los usuarios.
                    </p>
                </div>
                <ChatBubbleLeftRightIcon className="absolute right-12 bottom-0 h-64 w-64 text-white/5 -mb-20 pointer-events-none transform -rotate-12 group-hover:rotate-0 transition-transform duration-700" />
            </div>
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-slate-50/50">
                            <tr>
                                ...
                                <th className="px-3 py-4 text-left text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Usuario</th>
                                ...
                                <th className="px-3 py-4 text-left text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Asignado A</th>
                                ...
                                items.map((item) => (
                                    <tr key={item.id} className="hover:bg-indigo-50/30 transition-colors">
                                        ...
                                        <td className="px-3 py-4 text-sm font-medium text-slate-900">
                                            {item.creador?.nombre_completo || 'Desconocido'}
                                        </td>
                                        <td className="px-3 py-4 text-sm text-slate-600">{item.asunto}</td>
                                        ...
                                        <td className="px-3 py-4 text-sm text-slate-600">
                                            {item.asignado?.nombre_completo || <span className="text-gray-400 italic">Sin asignar</span>}
                                        </td>
                                            ...
                                            <div className="flex justify-end gap-2">
                                                {!item.asignado_a && (
                                                    <button 
                                                        onClick={async () => {
                                                            try {
                                                                await buzonSugerenciasService.asignarResponsable(item.id, user?.id);
                                                                fetchData();
                                                            } catch (error) {
                                                                console.error(error);
                                                            }
                                                        ...
                                                        title="Asignarme"
                                                        className="text-xs font-bold text-indigo-600 hover:text-indigo-900 transition-colors bg-indigo-50 hover:bg-indigo-100 rounded-full px-3 py-1 flex items-center justify-center"
                                                    >
                                                        Mío
                                                    </button>
                                                )}
